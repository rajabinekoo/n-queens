...
class HC:
    ...
    def start(self):
        ...
        self.steps = ((self.randSolution[1]) * (self.d - 1)) + \
            (abs(self.randSolution[0]-tempBoard[self.randSolution[1]]))
    ...
    def report(self):
        ...
        print("= = = = = = = = = = = = = = = = = = = =")
        finalBoard = self.constructBoard(self.board)
        return self.board, finalBoard
    ...
    def constructBoard(self, board):
        ...
        for i in range(0, len(board)):
            for j in range(len(finalBoard[i])):
                if finalBoard[i][j] == 'Q':
                    print(colored(finalBoard[i][j],
                          self.getColor(j, board)), end=" ")
                else:
    ...
    # def checkSuccess(self, board, finalBoard):
    #     for i in range(0, len(board)):
    #         for j in range(len(finalBoard[i])):
    #             if finalBoard[i][j] == 'Q':
    #                 for col in range(len(board)):
